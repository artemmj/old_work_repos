...
class CharValueSerializer(serializers.Serializer):
    value = serializers.CharField(source='*')  # noqa: WPS110


class NumberValueSerializer(serializers.Serializer):
    value = serializers.IntegerField(source='*')  # noqa: WPS110
...
class PointExtField(PointField):
    class Meta:
        swagger_schema_fields = {
            'type': openapi.TYPE_OBJECT,
            'title': 'Point',
            'properties': {
                'latitude': openapi.Schema(title='Широта', type=openapi.TYPE_NUMBER, format=openapi.FORMAT_FLOAT),
                'longitude': openapi.Schema(title='Долгота', type=openapi.TYPE_NUMBER, format=openapi.FORMAT_FLOAT),
            },
        }
