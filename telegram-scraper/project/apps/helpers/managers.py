...
class CustomFieldUserManager(UserManager):
    ...
    def _create_user(self, email, **fields):
        ...
        """
        if self.username_field_name not in fields:
            raise ValueError(f'The given {self.username_field_name} must be set')
        if self.password_field_name not in fields:
            raise ValueError(f'The given {self.password_field_name} must be set')
