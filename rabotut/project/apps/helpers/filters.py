...
class MultipleParamVersion(str, Enum):
    COMMA_SEPARATED = '1'
...
class MultipleParameterWidget(BaseCSVWidget):
    ...
    def value_from_datadict(self, data: Union[MultiValueDict, dict], files, name):
        ...
        if param_version == MultipleParamVersion.COMMA_SEPARATED:
            for item in data.getlist(name):
                if not isinstance(item, str):
                    continue
                values_list.extend(x.strip() for x in item.rstrip(',').split(','))
        elif param_version == MultipleParamVersion.DUPLICATE_PARAMETER:
            values_list = data.getlist(name)

        if len(values_list) > 0:
            values_list = list({x for x in values_list if x})
...
class ListFilter(django_filters.Filter):
    ...
    def filter(self, queryset, value):
        if value in EMPTY_VALUES:
            return queryset
        return super().filter(queryset, [item.strip() for item in value.split(',')])
...
class CustomDjangoFilterBackend(DjangoFilterBackend):
    def get_coreschema_field(self, filter_field: django_filters.Filter):
        ...
        if type(filter_field) is django_filters.ModelChoiceFilter:
    ...
    def make_single_field(self, filter_field: django_filters.Filter, *, main_kwargs: bool = True):
        ...
        if isinstance(filter_field, django_filters.DateFilter):
            ...
            return coreschema.Enum(
                **kwargs,
                enum=[choice[0] for choice in filter_field.extra.get('choices', [])],
            )
        return super().get_coreschema_field(filter_field)
