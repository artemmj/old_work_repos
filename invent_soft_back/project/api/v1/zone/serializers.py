...
class ZoneReadSerializer(EagerLoadingSerializerMixin, serializers.ModelSerializer):  # noqa: WPS214
    ...
    def get_barcode_amount(self, zone: Zone) -> int:
        return zone.barcode_amount

    @swagger_serializer_method(serializer_or_field=serializers.BooleanField)
    def get_is_empty(self, obj):
        return obj.tasks_scanned_products_count == 0

    @swagger_serializer_method(serializer_or_field=ZoneTaskEmployee(many=True))
    def get_counter_scan(self, zone: Zone):
        return [
            {'employee': {'username': counter_scan}}
            for counter_scan in zone.counter_scans
        ]

    @swagger_serializer_method(serializer_or_field=serializers.BooleanField)
    def get_is_counter_scan_count(self, zone: Zone):
        return False

    @swagger_serializer_method(serializer_or_field=ZoneTaskEmployee(many=True))
    def get_controller(self, zone: Zone):
        return [
            {'employee': {'username': controller}}
            for controller in zone.controllers
        ]

    @swagger_serializer_method(serializer_or_field=serializers.BooleanField)
    def get_is_controller_count(self, zone: Zone):
        return False
...
class ZoneBulkUpdateSelectSerializer(serializers.Serializer):
    project_id = serializers.PrimaryKeyRelatedField(queryset=Project.objects.all(), required=True)
    zones_ids = serializers.PrimaryKeyRelatedField(queryset=Zone.objects.all(), required=True, many=True)
    storage_name = serializers.CharField(required=True)


class ZoneBulkUpdateRangeSerializer(serializers.Serializer):
    project_id = serializers.PrimaryKeyRelatedField(queryset=Project.objects.all(), required=True)
