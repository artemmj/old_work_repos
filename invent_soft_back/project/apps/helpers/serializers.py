...
class TemplateFieldEnum(serializers.ChoiceField):
    class Meta:
        ...

    def __init__(self, enum_class, *args, **kwargs):  # noqa: D107
        TemplateFieldEnum.Meta.swagger_schema_fields = {
            'type': openapi.TYPE_OBJECT,
            'title': 'Type',
            'properties': {
                'name': openapi.Schema(
    ...
    def to_representation(self, value):  # noqa: WPS110
        return TemplateFieldSerializer(self.enum_class[value.upper()]).data if value else None


class TemplateFieldSerializer(serializers.Serializer):
    value = serializers.SerializerMethodField(method_name='rename_value')
    name = serializers.CharField(source='label')

    def rename_value(self, template_field_value: str):
        if 'pin_' in template_field_value:
            return template_field_value[:-3] + template_field_value[-3:].replace('_', '--')
