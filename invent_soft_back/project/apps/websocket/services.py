...
class SendWebSocketInfo:
    def __init__(self):
        self.channel_layer = layers.get_channel_layer()

    def send_about_update_zones(self, zones: List[Zone]):
        from apps.zone.services.get_zones import GetZonesService  # noqa: WPS433

        zones_ids = [zone.id for zone in zones]
        zones_with_virtual_attrs = GetZonesService().process().filter(id__in=zones_ids)
    ...
    def send_about_new_documents(self, documents: List[Document]):
        from api.v1.document.services.get_documents import GetDocumentsService  # noqa: WPS433

        documents_ids = [document.id for document in documents]
        documents_with_virtual_attrs = GetDocumentsService().process().filter(id__in=documents_ids)

        for document in documents_with_virtual_attrs:
