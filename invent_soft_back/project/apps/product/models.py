...
class ScannedProductQuerySet(models.QuerySet):
    ...
    def with_discrepancy_calculation(self):
        return self.annotate(
            discrepancy=models.Case(
                models.When(
                    product__remainder__lt=0,
                    then=Abs(models.F('product__remainder') - models.F('amount')),
                ),
                default=models.F('amount') - models.F('product__remainder'),
                output_field=models.IntegerField(),
            ),
            discrepancy_decimal=models.Case(
                models.When(
                    product__remainder__lt=0,
                    then=Abs(models.F('product__remainder') - models.F('amount')),
                ),
...
class Product(UUIDModel):
    ...
    vendor_code = models.CharField('Код товара', max_length=250, null=True, blank=True)
    barcode = models.CharField('Штрих-код', max_length=150)
