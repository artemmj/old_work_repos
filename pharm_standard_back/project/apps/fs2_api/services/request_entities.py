...
def get_all_entities(user, query_params=None, is_documents=True):
    ...
    if query_params and 'search' in query_params:
        ...
        for dict_ in documents_list:
            ...
            name_contains = query_params['search'].lower() in dict_['name'].lower()
            human_name_contains = query_params['search'].lower() in dict_['human_name'].lower()
            # Если есть параметр exclude, исключить их поиска то, что ввели
            if 'exclude' in query_params and query_params['exclude']:
                if not name_contains and not human_name_contains:
                    searched_list.append(dict_)
            # Иначе искать обычно
            else:
                if name_contains or human_name_contains:
                    searched_list.append(dict_)
