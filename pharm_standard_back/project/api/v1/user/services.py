...
def auth_via_pharm_apis(request):
    ...
    email = f'{username}@pharmstd.ru'

    # Проверка для тестового пользователя
    test_user = User.objects.filter(email=email)
    if test_user and test_user[0].is_test_user:
        user = test_user[0]
        ...
        try:
            Fs2ApiProxy(username, password)
        except Exception as original_exc:
            error_message = {
                'detail': 'Не удается авторизоваться в апи фс2, проверьте логин/пароль.',
                'source': email,
                'status': 400,
