...
from apps.fs2_api.services import (
    get_all_entities, get_detailed_entity, prepare_action, commit_action, rollback_action, send_attach_action,
    check_dss_status, get_statuses_from_documents, get_filtered_documents_by_status, get_fast_entity,
)
...
class DocumentViewSet(viewsets.ExtendedViewSet):
    ...
    @action(['get'], True, 'fast-retrieve', 'fast-retrieve')
    def fast_retrieve(self, request, pk=None):
        document = get_fast_entity(request.user, pk)
        if not document:
            raise NotFound('Документ не найден')
        return Response(document, status=status.HTTP_200_OK)
