...
class DocumentQuerySet(models.QuerySet):
    ...
    def with_color_numbers(self):
        return self.annotate(
            color_number=models.Case(
                models.When(color=DocumentColorChoices.RED, then=models.Value(1)),
                models.When(color=DocumentColorChoices.ORANGE, then=models.Value(2)),
                models.When(color=DocumentColorChoices.GREEN, then=models.Value(3)),
                models.When(color=DocumentColorChoices.BLUE, then=models.Value(4)),
                models.When(color=DocumentColorChoices.VIOLET, then=models.Value(5)),
                models.When(color=DocumentColorChoices.GRAY, then=models.Value(6)),
                models.When(color=DocumentColorChoices.WHITE, then=models.Value(7)),
            ),
        )
    ...
    def with_auditor_barcode_amount(self):
        return self.annotate(
            auditor_barcode_amount=models.Case(
                models.When(
                    auditor_task__isnull=False,
                    then=models.F('auditor_task__result'),
                ),
                default=0,
                output_field=models.DecimalField(),
            ),
    ...
    def with_auditor_controller_barcode_amount(self):
        return self.annotate(
            auditor_controller_barcode_amount=models.Case(
                models.When(
                    auditor_task__isnull=False,
                    then=models.F('auditor_controller_task__result'),
                ),
                default=0,
                output_field=models.DecimalField(),
            ),
...
class Document(CreatedModel, LifecycleModel):
    ...
    def generate_fake_id(self):
        ...
        SendWebSocketService().send_about_new_documents(documents=[self])

    @hook(AFTER_UPDATE)
    def send_websocket_signal(self):
        from apps.websocket.services import SendWebSocketService  # noqa: WPS433
        SendWebSocketService().send_about_update_documents(documents=[self])
