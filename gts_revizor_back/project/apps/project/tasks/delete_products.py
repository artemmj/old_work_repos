...
class DeleteProjectProductsService(AbstractService):  # noqa: WPS338
    ...
    def _delete_addit_attrs(self):
        cursor = connection.cursor()
        addit_attrs_pks = list(AdditionalProductTitleAttribute.objects.filter(
            project=self.project,
        ).values_list(
            'pk', flat=True,
        ))

        for chunk in self._chunks(lst=addit_attrs_pks, n=CHUNK_SIZE):
            delete_query = 'BEGIN;\r\n'
            for attr_id in chunk:  # noqa: WPS519
                delete_query += f"DELETE FROM product_additionalproducttitleattribute WHERE id = \'{attr_id}\';\r\n"
            delete_query += 'COMMIT;'
            cursor.execute(delete_query)
    ...
    def process(self):  # noqa: WPS231
        ...
        # Удалить все доп. атрибуты
        self._delete_addit_attrs()

        delete_products_pks = list(Product.objects.filter(
            project=self.project,
        ).exclude(
            title='Неизвестный товар',
        ).values_list(
        ...
        for chunk in self._chunks(lst=delete_products_pks, n=CHUNK_SIZE):
            self._make_and_execute_queries(chunk=chunk)
            current_task.update_state(
                state='PENDING',
                meta={
                    'current': delete_counter,
                    'total': len(delete_products_pks),
                    'percent': int(delete_counter / (len(delete_products_pks) / 100)),
                },
            )
            delete_counter += CHUNK_SIZE
