...
@pytest.fixture()
def project_suspicious_barcodes_report(
    project,
    products_factory,
    ...
    """Фикстура для создания проекта для Отчета по подозрительным ШК."""
    zones = zones_factory(
        count=3,
        project=project,
    ...
    documents_factory(
        ...
        suspicious=(suspicious for suspicious in [True, False, True]),
        status=(
            status
            for status
