import pytest
from mixer.backend.django import mixer

from apps.document.models import DocumentStatusChoices, DocumentColorChoices
from apps.employee.models import Employee, EmployeeRoleChoices
from apps.task.models import TaskTypeChoices, TaskStatusChoices
from apps.zone.models import ZoneStatusChoices

pytestmark = [pytest.mark.django_db]


@pytest.fixture()
def employee_for_auditor_report(
    project,
    products_factory,
    scanned_products_factory,
    documents_factory,
    zones_factory,
    tasks_factory,
):
    """Фикстура создания сотрудника для отчета Аудитора и Аудитора УК"""
    employee = mixer.blend(
        Employee,
        project=project,
        roles=[EmployeeRoleChoices.COUNTER, EmployeeRoleChoices.AUDITOR, EmployeeRoleChoices.AUDITOR_EXTERNAL],
    )
    zones = zones_factory(
        count=3,
        project=project,
        serial_number=(serial_number for serial_number in [1, 2, 3]),
        status=(status for status in [ZoneStatusChoices.READY, ZoneStatusChoices.NOT_READY, ZoneStatusChoices.READY]),
    )
    counter_scan_tasks = tasks_factory(
        count=3,
        zone=(zone for zone in zones),
        employee=employee,
        type=TaskTypeChoices.COUNTER_SCAN,
        result=50,
        status=TaskStatusChoices.SUCCESS_SCAN,
    )
    controller_tasks = tasks_factory(
        count=3,
        zone=(zone for zone in zones),
        employee=employee,
        type=TaskTypeChoices.COUNTER_SCAN,
        result=50,
        status=TaskStatusChoices.SUCCESS_SCAN,
    )
    auditor_tasks = tasks_factory(
        count=3,
        zone=(zone for zone in zones),
        employee=employee,
        type=TaskTypeChoices.AUDITOR,
        result=60,
        status=TaskStatusChoices.SUCCESS_SCAN,
    )
    auditor_controller_tasks = tasks_factory(
        count=3,
        zone=(zone for zone in zones),
        employee=employee,
        type=TaskTypeChoices.AUDITOR_CONTROLLER,
        result=70,
        status=TaskStatusChoices.SUCCESS_SCAN,
    )
    products = products_factory(
        count=3,
        project=project,
    )
    scanned_products_factory(
        count=3,
        product=(product for product in products),
        task=(task for task in auditor_tasks),
        amount=(amount for amount in [4, 6, 8])
    )
    documents_factory(
        count=3,
        employee=employee,
        zone=(zone for zone in zones),
        serial_number=(number for number in [1, 2, 3]),
        counter_scan_task=(task for task in counter_scan_tasks),
        controller_task=(task for task in controller_tasks),
        auditor_task=(task for task in auditor_tasks),
        auditor_controller_task=(task for task in auditor_controller_tasks),
        status=(
            status
            for status
            in [DocumentStatusChoices.READY, DocumentStatusChoices.NOT_READY, DocumentStatusChoices.READY]
        ),
        color=(color for color in [DocumentColorChoices.GREEN, DocumentColorChoices.RED, DocumentColorChoices.GREEN]),
    )

    return employee
