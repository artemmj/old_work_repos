...
from apps.project.tasks import (
    delete_products_celery_wrapper,
    export_project_celery_task_wrapper,
    export_settings,
    import_employees_celery_wrapper,
    import_products_celery_wrapper,
    import_project_celery_task_wrapper,
    import_settings,
    import_zones_celery_wrapper,
)
from apps.project.tasks.export_document_services.core import export_document_celery_wrapper
...
class ProjectViewSet(ExtendedModelViewSet, DeleteBatchMixin):  # noqa: WPS214
    ...
    def destroy(self, request, pk=None):
        ...
        if getattr(instance, 'rmm_settigns', False) and instance.rmm_settings:
            instance.rmm_settings.delete()
        if getattr(instance, 'terminal_settings', False) and instance.terminal_settings:
    ...
    def export_document(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        task = export_document_celery_wrapper.delay(serializer.data)
        return Response({'result_id': task.id}, status=status.HTTP_200_OK)
    ...
    def export_project(self, request, pk=None):
        ...
        host = f'{request.scheme}://{request.get_host()}'
        task = export_project_celery_task_wrapper.delay(host, pk)
        return Response({'result_id': task.id}, status=status.HTTP_200_OK)
    ...
    def import_project(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        task = import_project_celery_task_wrapper.delay(serializer.data['file'])
