...
from api.v1.sync.services.handle_terminal_session.process_auditor_external import ProcessAuditorExternalService
...
def test_process_auditor(task_for_process_auditor_external):
    """Тест для проверки сервиса обработки задания ВНЕШНИЙ АУДИТОР."""
    ...
    for srcuser in serializer_data.get('users'):
        for src_task in srcuser.get('tasks'):
            ...
            ProcessAuditorExternalService(src_task, task, zone_documents).process()

    task_for_process_auditor_external.refresh_from_db()
    document.refresh_from_db()

    assert task_for_process_auditor_external.status == TaskStatusChoices.WORKED
    assert document.auditor_external_task == task
