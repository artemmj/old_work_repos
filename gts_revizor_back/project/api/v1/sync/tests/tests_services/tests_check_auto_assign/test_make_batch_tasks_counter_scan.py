...
def test_make_batch_tasks_counter_scan(tasks_for_make_batch_tasks_counter_scan):
    """Тест для проверки функции _make_batch_tasks_counter_scan сервиса CheckAutoAssignService"""
    ...
    zone_2 = Zone.objects.filter(serial_number=2).first()
    ...
    for srcuser in serializer_data.get('users'):
        for src_task in srcuser.get('tasks'):
            ...
            CheckAutoAssignService(task)._make_batch_tasks_counter_scan(not_ready_zones)

    assert Task.objects.filter(zone__project=project).count() == 3
    assert zone_2.tasks.count() == 1
    assert zone_2.tasks.first().type == TaskTypeChoices.COUNTER_SCAN
