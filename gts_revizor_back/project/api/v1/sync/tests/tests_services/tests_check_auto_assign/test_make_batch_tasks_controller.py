...
def test_make_batch_tasks_controller(tasks_for_make_batch_tasks_controller):
    """Тест для проверки функции _make_batch_tasks_controller сервиса CheckAutoAssignService"""
    ...
    for srcuser in serializer_data.get('users'):
        for src_task in srcuser.get('tasks'):
            ...
            CheckAutoAssignService(task)._make_batch_tasks_controller(not_ready_zones)

    assert Task.objects.filter(zone__project=project).count() == 4
    assert zone_2.tasks.count() == 2
    assert zone_2.tasks.filter(type=TaskTypeChoices.CONTROLLER).count() == 1
