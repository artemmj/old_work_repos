...
from api.v1.sync.services.handle_terminal_session.process_auditor import ProcessAuditorService
...
def test_process_auditor(task_for_process_auditor):
    """Тест для проверки сервиса обработки задания АУДИТОР."""
    ...
    for srcuser in serializer_data.get('users'):
        for src_task in srcuser.get('tasks'):
            ...
            ProcessAuditorService(src_task, task, zone_documents).process()

    task_for_process_auditor.refresh_from_db()
    document.refresh_from_db()

    assert task_for_process_auditor.status == TaskStatusChoices.WORKED
    assert document.auditor_task == task
