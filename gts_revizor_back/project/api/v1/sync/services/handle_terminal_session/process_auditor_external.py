...
class ProcessAuditorExternalService(AbstractService):
    """Сервис для обработки задания ВНЕШНИЙ АУДИТОР."""
    ...
    def process(self):
        if Document.objects.filter(zone=self.db_task.zone, auditor_external_task=self.db_task):
            return
        ...
        for zone_doc in self.zone_docs:
            zone_doc.auditor_external_task = self.db_task
            zone_doc.save()
