...
class DocumentFilterSetService:  # noqa: WPS338 WPS214
    ...
    def counter_scan_eq_meth(self, queryset, name, value):
        return queryset.filter(counter_scan_task__result=value)

    def counter_scan_not_eq_meth(self, queryset, name, value):
        return queryset.exclude(counter_scan_task__result=value).exclude(counter_scan_task__result__isnull=True)

    def counter_scan_more_than_meth(self, queryset, name, value):
        return queryset.filter(counter_scan_task__result__gte=value)

    def counter_scan_less_than_meth(self, queryset, name, value):
        if value == 0:
            return queryset.filter(counter_scan_task__result=value)
        return queryset.filter(counter_scan_task__result__lte=value)

    # counter controller

    def controller_eq_meth(self, queryset, name, value):
        return queryset.filter(controller_task__result=value)

    def controller_not_eq_meth(self, queryset, name, value):
        return queryset.exclude(controller_task__result=value).exclude(controller_task__result__isnull=True)

    def controller_more_than_meth(self, queryset, name, value):
        return queryset.filter(controller_task__result__gte=value)

    def controller_less_than_meth(self, queryset, name, value):
        if value == 0:
            return queryset.filter(controller_task__result=value)
        return queryset.filter(controller_task__result__lte=value)
